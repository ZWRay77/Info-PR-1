...
int main()
{
    ...
    if (abfrage != 'j')
    {
        ...
        studentenListe.pushBack(student);
        student = Student(23456, "Walter Rodenstock", "15.10.1963", "W黮lnerstr.9");
        studentenListe.pushBack(student);
    }
    ...
    do
    {
        ...
        switch (abfrage)
        {
            ...
            //Datenelement mit spezifischer Matrikelnummer entfernen
            case '5':
            {
            	if(studentenListe.empty()){
            		std::cout<<"Die Liste ist leer!\n\n"<<std::endl;
            	...
            	std::cout<<"Bitte geben sie die Matrikelnummer an: "<<std::endl;
            	unsigned int matNrLoeschen;
            	std::cin>>matNrLoeschen;

            	try{
            		Student geloeschter=studentenListe.deleteMatNr(matNrLoeschen);
            		std::cout<<"Der folgende Student wurde gelöscht:\n\n"<<std::endl;
            		geloeschter.ausgabe();
            	}catch(int e){
            		if(e==99){
            			std::cout<<"Der Student wurde nicht gefunden...\n\n"<<std::endl;
            		}else{
            			std::cout<<"Fehler nr"<<e<<std::endl;
            			throw e;
            		}
            	}
            }
            ...
            //Datenelement am Anfang hinzufügen
            case '6':
            {
            	unsigned int matNr=0;
            	std::string name=" ";
